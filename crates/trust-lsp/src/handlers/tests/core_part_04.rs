...
"#;
    ...
    let conf = symbols
        .iter()
        .find(|s| s.name.contains("Conf"))
        .expect("Conf");
    // Find R as child of Conf
    let r = conf
        .children
        .as_ref()
        .unwrap()
        .iter()
        .find(|s| s.name.contains("R"))
        .expect("R");
    // Find Fast as child of R
    let fast = r
        .children
        .as_ref()
        .unwrap()
        .iter()
        .find(|s| s.name.contains("Fast"))
        .expect("Fast");
    // Find P1 as child of R
    let p1 = r
        .children
        .as_ref()
        .unwrap()
        .iter()
        .find(|s| s.name.contains("P1"))
        .expect("P1");

    assert_eq!(fast.kind, tower_lsp::lsp_types::SymbolKind::EVENT);
    assert_eq!(p1.kind, tower_lsp::lsp_types::SymbolKind::OBJECT);
}
