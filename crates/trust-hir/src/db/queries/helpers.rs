...
pub(in crate::db) fn persistence_from_modifiers(modifiers: &VarBlockModifiers) -> Option<SmolStr> {
    if modifiers.persistent.is_some() {
        Some(SmolStr::new("PERSISTENT"))
    } else if modifiers.retain.is_some() {
        Some(SmolStr::new("RETAIN"))
    } else if modifiers.non_retain.is_some() {
        Some(SmolStr::new("NON_RETAIN"))
    } else {
        None
    }
}

// [v4.2] children_with_tokens() вместо descendants_with_tokens()
pub(in crate::db) fn edge_from_var_decl(node: &SyntaxNode) -> Option<SmolStr> {
    for element in node.children_with_tokens() {
        if let Some(token) = element.into_token() {
            match token.kind() {
                SyntaxKind::KwREdge => return Some(SmolStr::new("R_EDGE")),
                SyntaxKind::KwFEdge => return Some(SmolStr::new("F_EDGE")),
                _ => {}
            }
        }
    }
