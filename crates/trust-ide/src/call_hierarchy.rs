...
/// Simplified call edge for external consumers.
#[derive(Debug, Clone)]
pub struct SimpleCallEdge {
    /// File containing the caller.
    pub caller_file_id: FileId,
    /// Caller symbol ID.
    pub caller_symbol_id: SymbolId,
    /// File containing the callee.
    pub callee_file_id: FileId,
    /// Callee symbol ID.
    pub callee_symbol_id: SymbolId,
    /// Line number of the call site (0-based).
    pub line: u32,
}
...
/// Collects all call edges across the given files.
///
/// If `file_ids` is empty, collects edges across all files in the database.
pub fn collect_project_call_edges(db: &Database, file_ids: Vec<FileId>) -> Vec<SimpleCallEdge> {
    let allowed = if file_ids.is_empty() {
        None
    } else {
        Some(file_ids.iter().copied().collect::<FxHashSet<FileId>>())
    };
    let edges = collect_call_edges_in_files(db, allowed.as_ref());
    edges
        .into_iter()
        .map(|edge| {
            let source = db.source_text(edge.caller.file_id);
            let line = offset_to_line(&source, edge.range.start().into());
            SimpleCallEdge {
                caller_file_id: edge.caller.file_id,
                caller_symbol_id: edge.caller.symbol_id,
                callee_file_id: edge.callee.file_id,
                callee_symbol_id: edge.callee.symbol_id,
                line,
            }
        })
        .collect()
...
fn offset_to_line(content: &str, offset: u32) -> u32 {
